...
//Only compile if USEMODULE += schedstatistics is in makefile
    ...
    for (int i = 0; i < MAXTHREADS; i++) {
        ...
        if (!t) continue;
        total0 += sched_pidlist[t->pid].runtime_us;
        if (t->priority == THREAD_PRIORITY_IDLE)
            idle0 += sched_pidlist[t->pid].runtime_us;
    }
    //Sleep for 1 second
    ...
    for (int i = 0; i < MAXTHREADS; i++) {
        thread_t *t = (thread_t *)sched_threads[i];
        if (!t) continue;
        total1 += sched_pidlist[t->pid].runtime_us;
        if (t->priority == THREAD_PRIORITY_IDLE)
            idle1 += sched_pidlist[t->pid].runtime_us;
    }
...
// CONFIG_THREAD_ANALYZER=y are in prj.conf
    ...
    //Get usage stats for all CPU 0 cycles
    struct k_thread_runtime_stats cpu0, cpu1;
    k_thread_runtime_stats_cpu_get(0, &cpu0);
    //Sleep for 1 second
    k_msleep(1000U);
    //Get usage stats for all CPU 1 cycles again
    k_thread_runtime_stats_cpu_get(0, &cpu1);
    //Calculate delta, populate metrics struct, and return the metrics struct
    uint64_t d_idle  = cpu1.idle_cycles - cpu0.idle_cycles;
    uint64_t d_total = cpu1.execution_cycles - cpu0.execution_cycles;
