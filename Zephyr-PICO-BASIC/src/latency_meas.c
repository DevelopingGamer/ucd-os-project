...
//Set the trigger pin to 14 and the capture pin to 15
#define TRIGGER_PIN     GPIO_PIN(0, 14)
#define CAPTURE_PIN     GPIO_PIN(0, 15)
...
//Wait up to 100 ms for the mutex to be available and 
// then lock it if it is, record the time in the task 
// recv list, and iterate the list index
static void *response_thread(void *arg) {
    (void)arg;
    for (uint32_t i = 0; i < LATENCY_SAMPLES; i++) {
        uint32_t start_wait = ztimer_now(ZTIMER_MSEC);
        bool locked = false;
        
        // Poll the mutex for up to 100ms
        while ((ztimer_now(ZTIMER_MSEC) - start_wait) < 100U) {
            if (mutex_trylock(&_irq_mutex)) {
                locked = true;
                break;
            }
        ...
        //If the mutex wasn't available for 100ms break out of the loop
        if (!locked) {
            break; 
        }
...
//Initialze two binary semaphores, set the initial count to 0 and the maximum
// count to 1
